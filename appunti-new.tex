...
\]
(la divisione per $N-1$ anziché per $N$ è una correzione che rende la stima più attendibile quando i dati sono pochi; è la stessa formula usata dai fogli di calcolo con \texttt{DEV.ST} e da \texttt{numpy} con \texttt{ddof=1}).
...
Un blocchetto di ferro di massa $m = (450,0 +- 0,1) g$, viene inserito in un cilindro graduato con dell'acqua all'interno. L'acqua inizialmente raggiunge il livello $V1 = ( 730 +- 1) cm^3$ mentre, dopo l'inserimento del blocchetto, raggiunge il volume $V2= (787 +-1 ) cm^3$. Determina la densità del blocchetto in $g/cm^3$.
\begin{minted}{python}
m=450
Dm=0.1
V1=730
...
L'output è:
\begin{verbatim}
d = (7.894736842105263 +- 0.27876269621421973) g/cm^3
\end{verbatim}
In forma corretta abbiamo: $d = (7,9 +- 0,3) g/cm^3$, coerente con la densità del ferro.

\end{testexample}

\begin{testexample}[\thetcbcounter \,Errore nel moto uniformemente accelerato]

Un'automobile, compie un sorpasso passando dalla velocità iniziale $V1=(89 +-1 )km/h$ alla velocità di $V2=(122 +-1) km/h$, percorrendo una distanza $S = (80,2 +- 0,1 ) m$.  Determina l'accelerazione col suo errore assoluto e scrivi la misura in forma corretta usando la formula: $a = (V2^2 -V1^2)/(2*S)$.
...
print(f"a = ({a} +-{Da}) m/s^2")
\end{minted}
Ecco l'output:
\begin{verbatim}
a = (3.3495543548536055 +-0.20717979592289554) m/s^2
\end{verbatim}
In forma corretta abbiamo $a = (3,3 +- 0,2) m/s^2$.
