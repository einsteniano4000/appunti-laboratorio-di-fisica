...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[>=stealth]
        ...
        \draw[->,very thick,ocre] (0.35,0) -- (2.9,0) node[midway,above]{$\vec F_{AB}$};
        \draw[->,very thick,blue!60!black] (-0.35,0) -- (-1.7,0) node[midway,below]{$\vec F_{BA}$};
        \node at (1.6,-0.9) {\footnotesize $A$ spinge $B$ verso destra $\;\Rightarrow\;$ $B$ spinge $A$ verso sinistra};
    \end{tikzpicture}
    \caption{Terzo principio: le due forze sono uguali e opposte, ma applicate a corpi diversi ($\vec F_{AB}$ su $B$, $\vec F_{BA}$ su $A$).}
