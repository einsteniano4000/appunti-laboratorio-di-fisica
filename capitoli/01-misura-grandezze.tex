...
\end{itemize}
I becchi grossi, rivolti verso il basso, servono per le misure \emph{esterne} (spessori, diametri di alberi); i due becchi sottili in alto per le misure \emph{interne} (diametri di fori); un'astina solidale al corsoio esce dal fondo dell'asta e serve per le misure di \emph{profondità}.

\begin{figure}[!htbp]
    \centering
    \resizebox{\linewidth}{!}{%
    \begin{tikzpicture}[x=1mm,y=1mm,>=stealth,line join=round,font=\footnotesize]
        % asta fissa
        \draw[thick,fill=gray!10] (0,0) rectangle (116,7);
        \foreach \i in {0,2,...,116}{\draw (\i,7)--(\i,5.4);}
        \foreach \i in {0,10,...,110}{\draw[thick](\i,7)--(\i,4.4);}
        % becchi fissi (sinistra): esterni in basso, interni in alto
        \draw[thick,fill=gray!18] (0,0)--(0,-20)--(6,-20)--(6,0)--cycle;
        \draw[thick,fill=gray!18] (0,7)--(0,16)--(5,16)--(5,7)--cycle;
        % astina di profondita'
        \draw[thick,fill=gray!18] (116,2) rectangle (144,5);
        % corsoio + nonio + becchi mobili
        \draw[thick,fill=gray!22] (58,-3) rectangle (90,12);
        \foreach \k in {0,...,10}{\draw (60+\k*1.9,5.4)--(60+\k*1.9,3);}
        \draw[thick,fill=gray!18] (58,0)--(58,-20)--(64,-20)--(64,0)--cycle;
        \draw[thick,fill=gray!18] (58,7)--(58,16)--(63,16)--(63,7)--cycle;
        \draw[thick,fill=white] (80,8) circle (1.8);
        % etichette
        \node at (32,12) {asta fissa (scala in mm)};
        \node at (74,20) {nonio};            \draw[->] (74,17.5)--(70,6);
        \node at (95,20) {vite di fissaggio}; \draw[->] (92,17.5)--(80,10);
        \node at (128,-9) {astina di profondità}; \draw[->] (128,-6.5)--(128,1.5);
        \node at (10,-26) {becchi per misure esterne};
        \node at (8,22) {becchi per misure interne};
    \end{tikzpicture}%
    }
    \caption{Schema di un calibro a corsoio. Il corsoio scorre sull'asta fissa; la misura si legge confrontando lo zero del nonio con la scala millimetrata.}
