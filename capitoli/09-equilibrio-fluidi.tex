...
\begin{figure}[!htbp]
    ...
    \begin{tikzpicture}[>=stealth,scale=1]
        ...
        % dinamometro: filo (entra nella molla, il tratto interno non e' disegnato)
        \draw[thick] (2.5,4.15) -- (2.5,2.3);
        % molla, disegnata sopra il filo
        \draw[thick,decorate,decoration={coil,aspect=0.6,segment length=4pt,amplitude=4.5pt}]
            (2.5,5.4) -- (2.5,3.9);
        % sospensione in alto
        \draw[thick] (2.5,5.75) -- (2.5,5.4);
        \draw[thick] (2.32,5.75) -- (2.68,5.75);
        % corpo immerso
...
Dove serve usa $g = \SI{9,81}{\newton\per\kilo\gram}$, $\rho_\text{acqua} = \SI{1000}{\kilo\gram\per\metre\cubed}$, $\rho_\text{acqua di mare} = \SI{1030}{\kilo\gram\per\metre\cubed}$, $\rho_\text{Hg} = \SI{13600}{\kilo\gram\per\metre\cubed}$, $p_0 = \SI{1,013e5}{\pascal}$, $\SI{1}{\mmHg} \approx \SI{133}{\pascal}$.

\subsection*{Pressione}

\begin{esercizio}
Un trattore di massa \SI{3200}{\kilo\gram} ha quattro ruote; l'impronta di ciascuna sul terreno è di circa \SI{600}{\centi\metre\squared}. Calcola la pressione che esercita sul terreno e confrontala con quella di un mezzo cingolato di pari massa, la cui superficie d'appoggio complessiva è \SI{1,5}{\metre\squared}. Perché nei terreni molli si usano i cingoli?\\
\risp{$p_\text{trattore} \approx \SI{1,3e5}{\pascal}$; \; $p_\text{cingolato} \approx \SI{2,1e4}{\pascal}$ (circa 6 volte minore)}
\end{esercizio}

\begin{esercizio}
Una ventosa da parete ha un'area di contatto di \SI{12}{\centi\metre\squared} e regge un oggetto di peso \SI{30}{\newton}. Quale deve essere, al massimo, la pressione dell'aria rimasta intrappolata sotto la ventosa?\\
\risp{la depressione rispetto all'atmosfera è almeno \SI{25}{\kilo\pascal}, cioè $p \le \SI{76}{\kilo\pascal}$}
\end{esercizio}

\begin{esercizio}
Su una lastra di ghiaccio la pressione massima sopportabile senza sfondarla è \SI{20}{\kilo\pascal}. Una persona di \SI{65}{\kilo\gram} può attraversarla camminando con gli scarponi (impronta \SI{300}{\centi\metre\squared} per il piede che appoggia)? E con gli sci (\SI{0,25}{\metre\squared} ciascuno)?\\
\risp{con gli scarponi $p \approx \SI{21}{\kilo\pascal} > \SI{20}{\kilo\pascal}$: no; con gli sci $p \approx \SI{2,6}{\kilo\pascal}$: sì}
\end{esercizio}

\subsection*{Legge di Stevino}

\begin{esercizio}
In una torre dell'acqua il serbatoio contiene acqua per un'altezza di \SI{5,0}{\metre} e la sua base è a \SI{22}{\metre} dal suolo. Qual è la pressione dell'acqua a un rubinetto al piano terra?\\
\risp{$p \approx \SI{2,6e5}{\pascal} \approx \SI{2,6}{\bar}$}
\end{esercizio}

\begin{esercizio}
Il profondimetro di un sub segna una pressione relativa di \SI{1,8}{\bar}. A quale profondità si trova, in mare?\\
\risp{$h \approx \SI{17,8}{\metre}$}
\end{esercizio}

\begin{esercizio}
Il vetro circolare di un oblò (diametro \SI{25}{\centi\metre}) di un batiscafo si trova a \SI{400}{\metre} di profondità in mare. Con quale forza l'acqua vi preme?\\
\risp{$F \approx \SI{2,0e5}{\newton}$}
\end{esercizio}

\begin{esercizio}
Un recipiente cilindrico alto \SI{40}{\centi\metre} contiene, dal basso, \SI{20}{\centi\metre} di mercurio e sopra \SI{20}{\centi\metre} di acqua. Qual è la pressione (relativa) sul fondo?\\
\risp{$p \approx \SI{2,9e4}{\pascal}$}
\end{esercizio}

\subsection*{Principio di Pascal}

\begin{esercizio}
Un martinetto idraulico ha i pistoni di diametro \SI{15}{\milli\metre} e \SI{12}{\centi\metre}. Se sul pistone piccolo si può spingere al più con \SI{300}{\newton}, qual è il carico massimo sollevabile?\\
\risp{$F_2 \approx \SI{1,9e4}{\newton}$}
\end{esercizio}

\begin{esercizio}
In una pressa idraulica il pistone piccolo (area \SI{4}{\centi\metre\squared}) scende di \SI{30}{\centi\metre} a ogni pompata. Quante pompate servono per far salire di \SI{6}{\centi\metre} il pistone grande (area \SI{200}{\centi\metre\squared})?\\
\risp{$n = 10$}
\end{esercizio}
...
\begin{esercizio}
Un tubo a U contiene acqua. In un ramo si versa dell'alcol ($\rho = \SI{790}{\kilo\gram\per\metre\cubed}$) che forma una colonna di \SI{15}{\centi\metre} sopra la superficie di separazione. Di quanto differiscono le altezze dei due peli liberi?\\
\risp{$\approx \SI{3,2}{\centi\metre}$ (più alto il ramo con l'alcol)}
\end{esercizio}

\begin{esercizio}
Da una macchina del caffè con due beccucci escono, nello stesso momento, due caffè in tazze di forma diversa. Perché il livello raggiunto è lo stesso?\\
\risp{le due tazze e i condotti formano vasi comunicanti con uno stesso liquido: il livello dipende solo dalla pressione, uguale nei due}
\end{esercizio}

\subsection*{Pressione atmosferica}

\begin{esercizio}
Un barometro a mercurio segna \SI{748}{\mmHg}. Esprimi questa pressione in pascal e in ettopascal.\\
\risp{$\approx \SI{9,97e4}{\pascal} \approx \SI{997}{\hecto\pascal}$}
\end{esercizio}

\begin{esercizio}
In una giornata di bassa pressione il barometro segna \SI{980}{\hecto\pascal} invece dei \SI{1013}{\hecto\pascal} abituali. Di quanto è più bassa la colonna di mercurio rispetto ai soliti \SI{76}{\centi\metre}?\\
\risp{$\approx \SI{2,5}{\centi\metre}$ in meno (colonna di circa \SI{73,5}{\centi\metre})}
\end{esercizio}

\begin{esercizio}
Un contenitore da \SI{5,0}{\litre} è chiuso da un coperchio di area \SI{200}{\centi\metre\squared}; al suo interno si abbassa la pressione fino a \SI{0,30}{atm}. Che forza tiene premuto il coperchio contro il contenitore?\\
\risp{$F \approx \SI{1,4e3}{\newton}$}
\end{esercizio}

\subsection*{Principio di Archimede}

\begin{esercizio}
Un blocco di sughero ($\rho = \SI{240}{\kilo\gram\per\metre\cubed}$) di volume \SI{2,0}{\litre} viene tenuto completamente immerso in acqua. Che forza occorre per mantenerlo sotto?\\
\risp{$F = S - P \approx \SI{15}{\newton}$}
\end{esercizio}

\begin{esercizio}
Una corona pesa \SI{8,5}{\newton} in aria e \SI{7,9}{\newton} immersa in acqua. Calcola la sua densità e stabilisci se può essere d'oro puro ($\rho_\text{oro} = \SI{19300}{\kilo\gram\per\metre\cubed}$).\\
\risp{$\rho \approx \SI{1,4e4}{\kilo\gram\per\metre\cubed}$: non è oro puro}
\end{esercizio}

\begin{esercizio}
Un pontone galleggiante ha sezione orizzontale \SI{6,0}{\metre\squared} e, a vuoto, pesca \SI{20}{\centi\metre}. Di quanto pescherà in più se vi si carica un container di \SI{3,0}{\tonne}? (Acqua dolce.)\\
\risp{$\Delta h = \SI{0,50}{\metre}$}
\end{esercizio}

\begin{esercizio}
Un palloncino di gomma di massa \SI{3,0}{\gram} viene gonfiato con elio ($\rho_\text{He} = \SI{0,18}{\kilo\gram\per\metre\cubed}$) fino a \SI{6,0}{\litre}. Sale o resta a terra? Con quale forza netta? (Aria: $\rho = \SI{1,2}{\kilo\gram\per\metre\cubed}$.)\\
\risp{sale, con una forza netta di circa \SI{0,03}{\newton}}
\end{esercizio}

\begin{esercizio}
Di un iceberg emerge dall'acqua di mare un volume di \SI{600}{\metre\cubed}. Qual è il volume totale dell'iceberg? ($\rho_\text{ghiaccio} = \SI{920}{\kilo\gram\per\metre\cubed}$, $\rho_\text{mare} = \SI{1025}{\kilo\gram\per\metre\cubed}$.)\\
\risp{$V \approx \SI{5900}{\metre\cubed}$}
\end{esercizio}
